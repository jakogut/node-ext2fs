...
async function readFile(path, options) {
  ...
  const fd = isFd(path) ? path : await bindingPromises.open(path,
  ...
  try {
    ...
    const buffer = Buffer.allocUnsafeSlow(size || kReadFileBufferLength);
    await read(fd, buffer, position, length, -1);
  } finally {
    if (path !== fd) {
      await close(fd);
    }
...
fs.lchown = function(path, uid, gid) {
  callback = maybeCallback(callback);
  await fs.open(path, constants.O_WRONLY | constants.O_NOFOLLOW)
  try {
    await fs.fchown(fd, uid, gid, callback);
  } finally {
    await close(fd);
  }
};
...
async function utimes(path, atime, mtime) {
  ...
  // pathCheck(path);
	unimplemented('futimes');
};

async function futimes(fd, atime, mtime) {
  // atime = toUnixTimestamp(atime);
  // mtime = toUnixTimestamp(mtime);
  unimplemented('utimes');
};

async function writeAll(fd, buffer, offset, length, position) {
  let written;
  while (written = await write(fd, buffer, offset, length, position)) {
    offset += written;
    length -= written;
    if (position !== null) {
      position += written;
...
class ReadStream extends Readable {
  ...
  async _read(n) {
    ...
    try {
      // the actual read.
      ({bytesRead, buffer}) =
        await read(this.fd, pool, pool.used, toRead, this.pos);
    } finally {
      if (this.autoClose)
        this.destroy();
      this.emit('error', er);
...
class WriteStream extends Writable {
  ...
  async function _write(data, encoding, cb) {
  ...
  async function _writev(data, cb) {
    if (typeof this.fd !== 'number')
      return this.once('open', function() {
...
async function writev(fd, chunks, position, callback) {
...
const fs = {
  access,
  fsPointer: binding.fsPointer,
  closeAllFileDescriptors: binding.closeAllFileDescriptors,
  openFiles: binding.openFiles,
  constants,
}
